...
def generate_docx_from_text(edited_text, subject, summary, nfa_type, table_data=None):
    ...
    try:
        ...
        # Add title - CENTERED as shown in image
        ...
        # Add body content - filter out status messages
        filtered_edited_text = edited_text
        unwanted_phrases = [
        ...
        for phrase in unwanted_phrases:
            if phrase in filtered_edited_text:
                filtered_edited_text = filtered_edited_text.replace(phrase, "").strip()
                print(f"Removed unwanted phrase from edited text: {phrase}", file=sys.stderr)
        
        body_paragraph = doc.add_paragraph(filtered_edited_text)
        body_paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
        # Reduce paragraph spacing for single page limit
        body_paragraph.paragraph_format.space_after = Pt(3)  # Reduced from 6
        ...
        # Add conclusion
        conclusion_para = doc.add_paragraph("The above proposal is submitted for approval.")
        conclusion_para.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
        # Reduce paragraph spacing for single page limit
        conclusion_para.paragraph_format.space_after = Pt(3)  # Reduced from 6
...
def main():
    ...
    try:
        ...
        # Filter out status messages and unwanted content
        filtered_body_text = body_text
        unwanted_phrases = [
            "NFA generated successfully",
            "Document generated successfully", 
            "NFA created successfully",
        ...
        for phrase in unwanted_phrases:
            if phrase in filtered_body_text:
                filtered_body_text = filtered_body_text.replace(phrase, "").strip()
                print(f"Removed unwanted phrase: {phrase}", file=sys.stderr)
        
        # Split by double newlines to get sections (Request paragraph + Bullet points)
        sections = [section.strip() for section in filtered_body_text.split('\n\n') if section.strip()]
        ...
        for i, section in enumerate(sections):
            ...
            # Skip empty sections or status messages
            if not section or any(phrase in section for phrase in unwanted_phrases):
                print(f"Skipping section {i+1}: empty or status message", file=sys.stderr)
                continue
    ...
    # Add conclusion line after table with safer approach
    try:
        ...
        print("✅ Conclusion added successfully", file=sys.stderr)
    except Exception as e:
